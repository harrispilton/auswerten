...
#tau_alpha=1
params= Parameters()
params.add('logD',value=-11.2,min=-15.05,max=-8.85)
...
for filename in sef:
	...
	#	liste = re.findall(r"[\w.][\f]+",data)
		if liste[0]=='#' or float(liste[3])>15:
			pass
		else:
...
#print 'ZONE=\t'+str(zone[1])+'\n\n'
	...
	if i == 1:
		pass
	elif filename=='85otp_250K.sef':
		minimize(residuals,params,args=(np.array(sqrtom[-3:]),np.array(r1[-3:])))
