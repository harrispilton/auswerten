...
params.add('a0', value=1e-3, min=1e-9,max=4.1,vary=True)
...
#files=glob.glob('otp/solidecho/26'+'*K.dat')
files.sort()
...
for (data,frmin,frmax) in zip(files,freqmins,freqmaxs):
	...
	betrag=[b/ar1 for b in betrag]
	maxfreq=max(freq)
	freq=[f/maxfreq for f in freq]
	freqs.append(freq)
	betrags.append(betrag)
	verts.append(list(zip(freq,betrag)))
	...
	if str(fitten)=='y':
		...
		fitverts.append(list(zip(freq,fit)))
	ar1=0.
	ar2=0.
	for i in range(0,len(betrag)-1):
		ar1=ar1+((betrag[i]-params['y0'].value)*(freq[i+1]-freq[i]))
		ar2=ar2+((fit[i]-params['y0'].value)*(freq[i+1]-freq[i]))
...
#		ar2=ar2+fi
	...
	#sdfo=raw_input('next')
poly=PolyCollection(verts,facecolors=[cc('r'),cc('g'),cc('y')])
poly.set_alpha(0.7)
ax.add_collection3d(poly,zs=temps,zdir='y')
ax.set_xlim3d(-1e0,1e0)
ax.set_ylim3d(248,335)
