...
def residual(params,xdata,ydata=None):
	...
	if ydata is None:
		return invrec(xdata,satval,t1,t0)
	return (ydata-invrec(xdata,satval,t1,t0))
def Lorentz(x,f0,hwhm,a0,y0=0):
	return (y0+a0/(1.+((x-f0)/hwhm)**2))
	#return (a0 * (0.5 * fwhm)/((x * f0)**2.0 + 0.25 * fwhm**2))
def invrec(t,satval,t1,t0):
	return -2*satval*(np.exp(-(t+t0)/t1))+satval
...
files=glob.glob('otp/invrec/'+'*K.dat')
files.sort()
p0=[2,1.0e4,1.0]
#a0_a=[]
#fwhm_a=[]
...
for data in files:
	...
	fit=residual(params,np.array(time))
	ax.plot(time,fit,label=str(temps[-1])+' Fit')
...
plt.legend()
plt.show()
with open('invrec.dat','w') as fout:
